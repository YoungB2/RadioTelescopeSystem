...
%The Radio Telescope is designed to provide a scalable and recreatable hardware and software system with the current intent of capturing, processing, and storing (WHATEVER WE'RE TRYING TO CAPTURE).  The hardware systems will provide a web based user interface from which new valid position data for the telescope can be entered and current readouts of the hardware will be found.  The software system will be developed to accept real-time input from the telescope's receiver, process the information for areas of interest and save these areas to both local and online storage.  The software and hardware systems are designed with the intent that further groups can add additional functionallity and improvements as well as being able to be created on a budget. 
\textnormal{}
...
%Several elements may constrain the systems used by the radio telescope.  There is a limit to how much space the actual telescope will have available for internal components(HDD, Power supply, etc).  There will also be a constraint on the processing power of the systems that are used as the we will be utilizing two raspberry pi boards to accomplish processing of receiver data and as the central control unit.  
